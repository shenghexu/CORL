...
def Run_Simulation(rand_seed_n):
    ...
    N_in = 20
    N_prefix = 20
    env = EP_Env(N_in, N_prefix, rep_time, rand_seed_n*10)

    RL_delays = np.zeros(10000)
    RL_gains = np.zeros(10000)
    RL_actions = np.zeros((10000, N_in, N_prefix))
    ...
    for episode in range(1):
        ...
        for step in range(10000*rep_time):
            ...
            d_o, d_e, d_r, s1 = env.env_step(np.reshape(a0, (N_in, N_prefix)))
            s1 = s1/s_max
            r1 = -1*d_r
            ...
            if step % rep_time ==0:
                ...
                RL_actions[step] = np.reshape(a0, (N_in, N_prefix))
